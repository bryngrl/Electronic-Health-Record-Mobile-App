...
const ADPIEScreen = ({ onBack, labId, patientName }: any) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <LinearGradient
        colors={isDarkMode ? ['#064E3B', '#065F46', '#047857'] : ['#0A8219', '#6CCA77', '#C8FFCF']}
        start={{ x: 0, y: 0.5 }}
        end={{ x: 1, y: 0.5 }}
      ...
      >
        ...
        >
          <Text style={[styles.viewBtnText, !alert && { color: theme.textMuted }]}>
            VIEW
          </Text>
          <Icon
            name="play-arrow"
            size={14}
            color={alert ? (isDarkMode ? '#4ADE80' : '#10B981') : theme.textMuted}
          />
        </TouchableOpacity>
      </LinearGradient>
...
const createStyles = (theme: any, commonStyles: any, isDarkMode: boolean) => StyleSheet.create({
  ...
  subtitle: { fontSize: 10, color: theme.textMuted, letterSpacing: 0.5 },
